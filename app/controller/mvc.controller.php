...
//		$pagina = $this->replace_content('/\#MAINCONTENT\#/ms' ,$html , $pagina);
    ...
    function load_register() {
        ...
        $html = $this->load_page('app/views/modules/m.register.php');
        $pagina = $this->replace_content('/\#CONTENT1\#/ms', $html, $pagina);
        $html = $this->load_page('app/views/modules/m.register.company.php');
        $pagina = $this->replace_content('/\#RESULTS\#/ms', $html, $pagina);
        $html = $this->load_page('app/views/modules/m.register.contact.php');
        $pagina = $this->replace_content('/\#RESULTS1\#/ms', $html, $pagina);
        $this->view_page($pagina);
    }
    ...
    function add_company($vars, $table) {
        $oMySQL = new MySQL('sis_sa', 'root', 'root', 'localhost');
        if ($oMySQL->Insert($vars, $table) == TRUE) {
            $this->load_register();
            echo '<script type="text/javascript">alert("Datos guardados correctamente");</script>';
            $oMySQL->CloseConnection();
        } else {
            $this->load_register();
            echo '<script type="text/javascript">alert("No se pudieron ingresar los datos");</script>';
            return;
        }
    }
